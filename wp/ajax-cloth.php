    ...
    if(array_key_exists($collection, CFS()->get('collection'))){
        ...
        if( ! empty($fields) ){
            foreach ( $fields as $field ){
                if( array_key_exists($marker, $field['marker'])){
                    $item = array();
                    $price = 0;
...
<div class="cloth__item" data-price="<?php echo $k;?>" data-ids='<?php echo $val['ids'] ;?>'>
    <picture>
      <source srcset="<?php echo $val['webp'];?>" type="image/webp" alt="Студия авторской мебели WheelHouse">
...
<?php } else {?>
<p class="cloth__link">Мы не смогли автоматически подобрать подходящую ткань. Вы можете оформить <a href="https://wheelhousedesign.ru/nestandartnye-resheniya/">индивидуальный заказ</a></p>
<?php } ?>
