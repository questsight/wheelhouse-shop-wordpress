...
global $query_string;
query_posts($query_string.'post_type=product&product_cat=chehly-na-matras&posts_per_page=-1');
...
<script>
  ...
  jQuery('#choice-mattress').on('click', function(){
    ...
    if(jQuery('.variation_id').attr('data-chehly-na-matras')){
      var chehlyNaMatras = parseInt(jQuery('.variation_id').attr('data-chehly-na-matras'), 10);
    }else{
      var chehlyNaMatras = 0;
    }
    var dp = + jQuery('.summary .price .woocommerce-Price-amount bdi').text().replace(/\₽.*/, '').replace(/\s+/g, '') - chehlyNaMatras;
    var rp = + jQuery('.summary .price del .woocommerce-Price-amount bdi').text().replace(/\₽.*/, '').replace(/\s+/g, '') - chehlyNaMatras;
    var sp = + jQuery('.summary .price ins .woocommerce-Price-amount bdi').text().replace(/\₽.*/, '').replace(/\s+/g, '') - chehlyNaMatras;
    jQuery('.variation_id').attr('data-chehly-na-matras',price);
    ...
    jQuery('#product__chehly-na-matras').parent('.product__variation').addClass('choice');
    jQuery('#product__chehly-na-matras').parent('.product__variation').children('.product__delete').removeClass('hidden');
  });
</script>