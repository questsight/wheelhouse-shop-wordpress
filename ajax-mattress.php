...
if ($_REQUEST && !empty($_REQUEST) && $_REQUEST['mattress-size']) {
  ...
  if ($product->is_type( 'variable' )){
    ...
    foreach ($available_variations as $key => $value){
      ...
      if($value['attributes']['attribute_pa_size-krd']==$_REQUEST['mattress-size']&&$value['dimensions']['height']<=25&&$value['dimensions']['height']>=12&&!$kr || $value['attributes']['attribute_pa_size-krd']==$_REQUEST['mattress-size']&&$kr&&$value['dimensions']['height']>15){
        $fields = CFS()->get( 'mattress-hardness' );
        if( ! empty($fields) ){
  ...
  foreach ( $mattress as $key => $value ){
    ...
    echo '<div style="border:1px solid rgba(171, 178, 204, 0.85);" class="listing__item mattress__item flip"><div class="flip__front"><img src="'.$value['img'].'"><div class="listing__title" style="text-transform:none;"><span id="title-name">'.$value['name'].'</span><br>Высота: '.$value['height'].'см<br>'.$value['hardness'].'</div><span class="listing__price"><del>'.number_format($price, 0, ',', ' ').' ₽</del><ins>'.number_format(round($price*0.9,-2), 0, ',', ' ').' ₽</ins></span></div><div class="flip__back" data-price="'.round($price*0.9,-2).'" data-ids="'.$value['variation_id'].'" data-mid="'.$value['main_id'].'">'.$value['description'].'</div></a></div>';
  }
...
<script>
  ...
  jQuery('#choice-mattress').on('click', function(){
    ...
    if(jQuery('.variation_id').attr('data-mattress')){
      var mattress = parseInt(jQuery('.variation_id').attr('data-mattress'), 10);
    }else{
      var mattress = 0;
    }
    var dp = + jQuery('.summary .price .woocommerce-Price-amount bdi').text().replace(/\₽.*/, '').replace(/\s+/g, '') - mattress;
    var rp = + jQuery('.summary .price del .woocommerce-Price-amount bdi').text().replace(/\₽.*/, '').replace(/\s+/g, '') - mattress;
    var sp = + jQuery('.summary .price ins .woocommerce-Price-amount bdi').text().replace(/\₽.*/, '').replace(/\s+/g, '') - mattress;
    jQuery('.variation_id').attr('data-mattress',price);
