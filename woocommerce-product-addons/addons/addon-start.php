...
<div class="wc-pao-addon-container <?php echo $required ? 'wc-pao-required-addon' : ''; ?> wc-pao-addon wc-pao-addon-<?php echo sanitize_title( $name ); echo sanitize_title( $name )=="vybrat-czvet-opor"? ' hidden':'';?>" data-product-name="<?php echo esc_attr( $product_title ); ?>">
	...
	if ( $name ) {
		...
		<?php
			...
			switch ( $title_format ) {
				case 'heading':
					?>
					<p class="wc-pao-addon-name" data-addon-name="<?php echo esc_attr( wptexturize( $name ) ); ?>" data-has-per-person-pricing="<?php echo esc_attr( $has_per_person_pricing ); ?>" data-has-per-block-pricing="<?php echo esc_attr( $has_per_block_pricing ); ?>"><?php echo wptexturize( $name ); ?> <?php echo $required ? '<em class="required" title="' . __( 'Required field', 'woocommerce-product-addons' ) . '"></em>&nbsp;' : ''; ?><?php echo wp_kses_post( $price_display ); ?></p>
					<?php
					break;
				case 'hide':
					?>
