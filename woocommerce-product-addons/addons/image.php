...
<?php if ( empty( $required ) ) { ?>
	<a href="#" title="<?php echo esc_attr__( '', 'woocommerce-product-addons' ); ?>" class="wc-pao-addon-image-swatch" data-value="" data-price="">
...
<?php foreach ( $addon['options'] as $i => $option ) {
	...
	?>
		<a href="#" title="<?php echo esc_attr( $option['label'] . ' ' . $price_tip ); ?>" class="wc-pao-addon-image-swatch" data-value="<?php echo sanitize_title( $option['label'] ) . '-' . $loop; ?>" data-price="<?php echo esc_attr( '<span class="wc-pao-addon-image-swatch-price">' . wptexturize( $option['label'] ) . ' ' . $price_display . '</span>' ); ?>" data-item="<?php echo wptexturize( $option['label'] );?>">
			<img src="<?php echo esc_url( $image_src[0] ? $image_src[0] : wc_placeholder_img_src() ); ?>" />
		</a>
<?php } ?>

<select class="wc-pao-addon-image-swatch-select wc-pao-addon-field product__variation-choice" name="addon-<?php echo sanitize_title( $field_name ); ?>">
	<?php if ( empty( $required ) ) { ?>
		<option value=""><?php esc_html_e( '', 'woocommerce-product-addons' ); ?></option>
	<?php } else { ?>
